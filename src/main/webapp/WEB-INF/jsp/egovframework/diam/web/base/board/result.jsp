...
<script type="text/javascript">
	<c:choose>
		<c:when test="${message == 'board.success.write'}">
			...
			document.location.href = "<c:out value='${root }'/>/index.do?<c:out value='${param.writeQueryString}' escapeXml='false' />";
		</c:when>
		...
		<c:when test="${message == 'board.success.delete'}">
			...
			document.location.href = "<c:out value='${root }'/>/index.do?" + unescapeHtml("<c:out value='${deleteSearchMap}' escapeXml='false' />");
		</c:when>
		
		<c:when test="${message == 'board.comment.delete'}">
			alert("댓글이 정상적으로 삭제되었습니다.");
			document.location.href = "<c:out value='${root }'/>/index.do?command=view&wr_id=<c:out value='${wr_id}'/>&<c:out value='${deleteSearchMap}' escapeXml='false' />";
		</c:when>		
		
		<c:when test="${message == 'board.list.auth'}">
			alert("글목록 권한이 없습니다.");
			document.location.href = "<c:out value='${root }'/>/index.do";
		</c:when>
		
		<c:when test="${message == 'board.read.auth'}">
			alert("글 상세보기 권한이 없습니다.");
			document.location.href = "<c:out value='${root }'/>/index.do?contentId=<c:out value='${dm_uid}'/>";
		</c:when>	
