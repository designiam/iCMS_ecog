...
function fnNew() {
	window.open("/adm/delete_write_form.do","new_write",option);
}
...
function fnRemove(){
    ...
    if (rows.length > 0) {
    	$.messager.confirm("", "정말 완전삭제하시겠습니까?", function (r) {
    		if (r) {
    			for(var i=0; i<rows.length; i++){
                    ids.push(rows[i].wr_id);
                    console.log(rows[i].wr_id);
                }
    			
    			$.ajax({
    				url: "/adm/delete_write_remove.do",
    				data: {wr_id : ids},
    				type: "post"
        ...
        $.messager.alert('경고', "완전삭제할 항목을 선택해주세요", 'warning');
    }
}
...
function fnRestore(){
    ...
    var rows = $('#dg').datagrid('getChecked');
    if (rows.length > 0) {
    	$.messager.confirm("", "정말 복구하시겠습니까?", function (r) {
    		if (r) {
    			for(var i=0; i<rows.length; i++){
                    ids.push(rows[i].wr_id);
                }
    			...
    			$.ajax({
    				url: "/adm/delete_write_restore.do",
    				data: {wr_id : ids},
    				type: "post"
    			}).done(function(response){
    				$.messager.alert('경고', response.notice , 'warning');
        ...
        $.messager.alert('경고', "복구할 항목을 선택해주세요", 'warning');
    }
}
function fnMove() {
	var ids = [];
...
$(function () {
    ...
    $("#fnRestore").off().on('click', function () {
    	fnRestore();
    });
    ...
    /*$(document).on('click', ".open_form", function () {
        ...
        window.open('/adm/delete_write_form.do?wr_id='+wr_id+'&wr_board='+wr_board,"modify_write",option);
    });
...
<div class="easyui-layout" fit="true">
    ...
    <div data-options="region:'center', border:false" class="Contents">
        <table id="dg"
               ...
               data-options="pagePosition:'top',rownumbers:true,pagination:true,singleSelect:true,url:'/adm/get_delete_list.do',method:'post',fitColumns:true,striped:false,selectOnCheck:false,checkOnSelect:false" pageList="[10,20,30,50,70,100]" pageSize="50" fit="true">
            <thead>
            <tr>
                ...
                <th data-options="field:'wr_is_notice',width:50,align:'center'">공지글여부</th>
                ...
                <th data-options="field:'dm_delete_id',width:80,align:'center'">삭제자</th>
                <th data-options="field:'dm_delete_dt',width:100,align:'center'">삭제일</th>
    ...
    <div data-options="region:'south'">
        <dl class="Tbottom">
            <dd>
                <button class="btn" id="fnRemove">완전삭제</button>
                <button class="btn" id="fnRestore">복구</button>
            </dd>
            <dd>
