...
public class Dm_member_vo implements Serializable {
	...
	/** 회원 아이디 */
	@NotNull(groups= {MemberAdminGroup.class, MemberUserGroup.class}, message="아이디를 입력해주세요.")
	@Pattern(groups= {MemberAdminGroup.class, MemberUserGroup.class}, regexp="^[a-z]{1}[a-z0-9]{4,19}$", message="아이디 형식에 맞지않습니다. 영문소문자로 시작하여 영문, 숫자로만 이루어진 5 ~ 20자 이하의 값을 입력해주세요.")
	...
	/** 회원 이름 */
	@NotNull(groups= {MemberAdminGroup.class}, message="이름을 입력해주세요.")
	@Pattern(groups= {MemberAdminGroup.class, MemberUserGroup.class}, regexp="^[a-zA-Z가-힣ㄱ-ㅎ]*$", message="닉네임은 한글, 영문으로 1자이상 20자 이하로 입력가능합니다.")
	@Size(groups= {MemberAdminGroup.class, MemberUserGroup.class}, min=1, max=30, message="이름은 1자 이상 20자 이하로 입력해주세요.")
